...
function processCacheData() {
    ...
    // Process each market in cache (soft liquidations)
    Object.keys(cacheData).forEach(key => {
        ...
        marketData.results.forEach(snapshot => {
            ...
            // Don't calculate hard liquidations from snapshot - they come from separate database
...
function updateFundsSavedTotal() {
    ...
    Object.keys(cacheData).forEach(key => {
        ...
        marketData.results.forEach(snapshot => {
            ...
            if (snapshotDate >= dateFrom && snapshotDate <= dateTo) {
                totalLiquidationProtection += snapshot.total_collateral_usd || 0;
            }
        });
    });
...
function updateFundsSavedByNetwork() {
    ...
    Object.keys(cacheData).forEach(key => {
        ...
        marketData.results.forEach(snapshot => {
            ...
            if (snapshotDate >= dateFrom && snapshotDate <= dateTo) {
                dataByNetwork[network].liquidationProtection += snapshot.total_collateral_usd || 0;
            }
        });
    });
